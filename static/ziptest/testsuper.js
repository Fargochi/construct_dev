...
test[test.length-1].push_question_and_backcolor("Сколько будет 2*2?","#55e326");
test[test.length-1].push_answer("Думаю, 4","1");
test[test.length-1].push_answer("Маловероятно, но, быть может, миллион?","0");
test[test.length-1].push_answer("Возможно, 6?","0");
test[test.length-1].push_answer("Все знают, что 15","0");
test.push(new constructor_slide);
